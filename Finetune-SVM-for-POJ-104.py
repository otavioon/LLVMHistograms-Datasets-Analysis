...
def search_best_SVM_model(X_train, X_val, y_train, y_val):
    ...
    for kernel in ['poly', 'rbf', 'sigmoid', 'precomputed']:
        for C in [1000.0, 10000.0, 20000.0, 50000.0, 100000.0]:
            for degree in [1, 2, 3, 4]: 
                if degree !=1 and kernel != 'poly': continue
                try:
                    model = svm.SVC(kernel=kernel, gamma='scale', C=C, degree=degree)
                    acc, f1, model = train_model(X_train, X_val, y_train, y_val, model)
                    verbose(2, "SVM: val acc: {}: C : {} : kernel : {} : degree : {}".format(acc,C,kernel,degree))
                    if not best_params or best_params["Val acc"] < acc:
                        best_params = { "Val acc": acc, "F1": f1, "params" : {"C":C, "kernel":kernel}, "model" : model}
                except:
                    verbose(2, "SVM: error  : C : {} : kernel : {} : degree : {}".format(C,kernel,degree))
    return best_params
