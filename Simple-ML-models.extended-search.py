...
def train_model(X_train, X_val, y_train, y_val, model):
    ...
    return acc, f1, model, training_time, validation_time
...
def search_best_KNN_model(X_train, X_val, y_train, y_val):
    ...
    for n_neighbors in [5, 10, 15, 20, 50]:
        for weights in ["uniform", "distance"]:
            ...
            acc, f1, model,t_time,v_time = train_model(X_train, X_val, y_train, y_val, model)
            if not best_params or best_params["Val acc"] < acc:
                best_params = { "Val acc": acc, "F1": f1, "Training Time":t_time, "Validation Time":v_time,  "params" : {"weights":weights, "n_neighbors":n_neighbors} , "model" : model}
    if best_params: best_params["Search Time"] = time.time() - search_start
    return best_params
        ...
        for solver in ["newton-cg", "lbfgs", "liblinear", "sag", "saga"]:
            for c in [5, 10, 20, 50, 70, 100]:
                for tol in [0.0001, 0.1, 0.5, 1.0]:
                    ...
                    acc, f1, model, t_time, v_time = train_model(X_train, X_val, y_train, y_val, model)
                    if not best_params or best_params["Val acc"] < acc:
                        best_params = { "Val acc": acc, "F1": f1, "Training Time":t_time, "Validation Time":v_time, "params" : {"C":C, "tol":tol, "penalty":penalty, "solver":solver} , "model" : model}
    if best_params: best_params["Search Time"] = time.time() - search_start
    return best_params
    ...
    for loss in ["log", "hinge", "modified_huber", "perceptron"]:
        for penalty in ["l1", "elasticnet", "l2"]:
            for max_iter in [50]:
                ...
                if not best_params or best_params["Val acc"] < acc:
                    best_params = { "Val acc": acc, "F1": f1,  "Training Time":t_time, "Validation Time":v_time, "params" : {"loss":loss, "penalty":penalty, "max_iter":max_iter} , "model" : model}
    return best_params
...
def evaluate_dataset(ds_name, train_filename, val_filename, test_filename, normalize=False):
    ...
    acc, f1, dt_model,t_time,v_time = train_model(X_train, X_val, y_train, y_val, tree.DecisionTreeClassifier())
    best_DT = { "Val acc" : acc, "Val f1" : f1, "Training Time":t_time, "Validation Time":v_time, "params" : "Default" , "model": dt_model}
